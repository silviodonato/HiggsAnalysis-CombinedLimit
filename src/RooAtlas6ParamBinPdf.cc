...
#include "../interface/RooAtlas6ParamBinPdf.h"
...
ClassImp(RooAtlas6ParamBinPdf)
//---------------------------------------------------------------------------
RooAtlas6ParamBinPdf::RooAtlas6ParamBinPdf(const char *name, const char *title,
				   RooAbsReal& _th1x,  
...
//---------------------------------------------------------------------------
RooAtlas6ParamBinPdf::RooAtlas6ParamBinPdf(const RooAtlas6ParamBinPdf& other, const char* name) :
...
//---------------------------------------------------------------------------
void RooAtlas6ParamBinPdf::setTH1Binning(TH1* _Hnominal){
...
//---------------------------------------------------------------------------
void RooAtlas6ParamBinPdf::setRelTol(double _relTol){
  relTol = _relTol;
}
//---------------------------------------------------------------------------
void RooAtlas6ParamBinPdf::setAbsTol(double _absTol){
  absTol = _absTol;
}
//---------------------------------------------------------------------------
Double_t RooAtlas6ParamBinPdf::evaluate() const
{
...
// //---------------------------------------------------------------------------
Int_t RooAtlas6ParamBinPdf::getAnalyticalIntegral(RooArgSet& allVars, RooArgSet& analVars, const char* rangeName) const{
  if (matchArgs(allVars, analVars, th1x)) return 1;
  return 0;
}

// //---------------------------------------------------------------------------
Double_t RooAtlas6ParamBinPdf::analyticalIntegral(Int_t code, const char* rangeName) const{
  ...
  ig.SetFunction(func,false);
   ...
   if (code==1 && iBinMin<=0 && iBinMax>=xBins){
     ...
     cout << "WARNING IN RooAtlas6ParamBinPdf: integration code is not correct" << endl;
