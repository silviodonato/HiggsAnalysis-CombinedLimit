...
#include "../interface/RooDijet4ParamSilvioPdf.h"
...
ClassImp(RooDijet4ParamSilvioPdf)
//---------------------------------------------------------------------------
RooDijet4ParamSilvioPdf::RooDijet4ParamSilvioPdf(const char *name, const char *title,
				   RooAbsReal& _th1x,  
...
//---------------------------------------------------------------------------
RooDijet4ParamSilvioPdf::RooDijet4ParamSilvioPdf(const RooDijet4ParamSilvioPdf& other, const char* name) :
...
//---------------------------------------------------------------------------
void RooDijet4ParamSilvioPdf::setTH1Binning(TH1* _Hnominal){
...
//---------------------------------------------------------------------------
void RooDijet4ParamSilvioPdf::setRelTol(double _relTol){
  relTol = _relTol;
}
//---------------------------------------------------------------------------
void RooDijet4ParamSilvioPdf::setAbsTol(double _absTol){
  absTol = _absTol;
}
//---------------------------------------------------------------------------
Double_t RooDijet4ParamSilvioPdf::evaluate() const
{
  ...
  Dijet4ParamSilvioFunction func;
  double params[7];
...
// //---------------------------------------------------------------------------
Int_t RooDijet4ParamSilvioPdf::getAnalyticalIntegral(RooArgSet& allVars, RooArgSet& analVars, const char* rangeName) const{
  if (matchArgs(allVars, analVars, th1x)) return 1;
  return 0;
}

// //---------------------------------------------------------------------------
Double_t RooDijet4ParamSilvioPdf::analyticalIntegral(Int_t code, const char* rangeName) const{
  ...
  ig.SetFunction(func,false);
   ...
   if (code==1 && iBinMin<=0 && iBinMax>=xBins){
     ...
     cout << "WARNING IN RooDijet4ParamSilvioPdf: integration code is not correct" << endl;
