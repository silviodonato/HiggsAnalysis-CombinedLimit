...
#include "../interface/RooModExp6ParamBinPdf.h"
...
ClassImp(RooModExp6ParamBinPdf)
//---------------------------------------------------------------------------
RooModExp6ParamBinPdf::RooModExp6ParamBinPdf(const char *name, const char *title,
				   RooAbsReal& _th1x,  
...
//---------------------------------------------------------------------------
RooModExp6ParamBinPdf::RooModExp6ParamBinPdf(const RooModExp6ParamBinPdf& other, const char* name) :
...
//---------------------------------------------------------------------------
void RooModExp6ParamBinPdf::setTH1Binning(TH1* _Hnominal){
...
//---------------------------------------------------------------------------
void RooModExp6ParamBinPdf::setRelTol(double _relTol){
  relTol = _relTol;
}
//---------------------------------------------------------------------------
void RooModExp6ParamBinPdf::setAbsTol(double _absTol){
  absTol = _absTol;
}
//---------------------------------------------------------------------------
Double_t RooModExp6ParamBinPdf::evaluate() const
{
...
// //---------------------------------------------------------------------------
Int_t RooModExp6ParamBinPdf::getAnalyticalIntegral(RooArgSet& allVars, RooArgSet& analVars, const char* rangeName) const{
  if (matchArgs(allVars, analVars, th1x)) return 1;
  return 0;
}

// //---------------------------------------------------------------------------
Double_t RooModExp6ParamBinPdf::analyticalIntegral(Int_t code, const char* rangeName) const{
  ...
  ig.SetFunction(func,false);
   ...
   if (code==1 && iBinMin<=0 && iBinMax>=xBins){
     ...
     cout << "WARNING IN RooModExp6ParamBinPdf: integration code is not correct" << endl;
