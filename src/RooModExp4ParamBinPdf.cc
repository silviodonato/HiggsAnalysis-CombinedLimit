...
#include "../interface/RooModExp4ParamBinPdf.h"
...
ClassImp(RooModExp4ParamBinPdf)
//---------------------------------------------------------------------------
RooModExp4ParamBinPdf::RooModExp4ParamBinPdf(const char *name, const char *title,
				   RooAbsReal& _th1x,  
...
//---------------------------------------------------------------------------
RooModExp4ParamBinPdf::RooModExp4ParamBinPdf(const RooModExp4ParamBinPdf& other, const char* name) :
...
//---------------------------------------------------------------------------
void RooModExp4ParamBinPdf::setTH1Binning(TH1* _Hnominal){
...
//---------------------------------------------------------------------------
void RooModExp4ParamBinPdf::setRelTol(double _relTol){
  relTol = _relTol;
}
//---------------------------------------------------------------------------
void RooModExp4ParamBinPdf::setAbsTol(double _absTol){
  absTol = _absTol;
}
//---------------------------------------------------------------------------
Double_t RooModExp4ParamBinPdf::evaluate() const
{
...
// //---------------------------------------------------------------------------
Int_t RooModExp4ParamBinPdf::getAnalyticalIntegral(RooArgSet& allVars, RooArgSet& analVars, const char* rangeName) const{
  if (matchArgs(allVars, analVars, th1x)) return 1;
  return 0;
}

// //---------------------------------------------------------------------------
Double_t RooModExp4ParamBinPdf::analyticalIntegral(Int_t code, const char* rangeName) const{
  ...
  ig.SetFunction(func,false);
   ...
   if (code==1 && iBinMin<=0 && iBinMax>=xBins){
     ...
     cout << "WARNING IN RooModExp4ParamBinPdf: integration code is not correct" << endl;
